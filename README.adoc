...
When prompted for theme name, typography, etc., just choose the defaults.

This process will install a browser animations module, and you'll need to import additional Material modules in `app.module.ts`:
...
Add a few styles to make things look right in `src/styles.css`:
...
Change your HTML to use Material components. For example, change `<h1>` in `app.component.html` to be `<mat-toolbar>`.

[[angular-material]]
.Angular Material
image::src/assets/images/angular-material.png[Angular Material, 800, scaledwidth="100%"]

The HTML templates and tests in the `angular-material` branch have been changed to use Angular Material. You can download them using the following command:

----
git clone -b angular-material https://github.com/mraible/ng-demo.git
----
