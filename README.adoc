...
Create `src/app/shared/auth/okta.auth.wrapper.ts` to wrap the Okta Auth SDK and integrate it with `OAuthService`. This `login()` method uses `OktaAuth` to get a session token and exchange it for ID and access tokens.
