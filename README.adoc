...
Change the `outputPath` in `angular.json` to put all files in the `dist` directory.
...
Add an `electron` script to start the Electron app after building the Angular project:
...
[[angular-electron]]
.Angular in Electron
image::src/assets/images/angular-electron.png[Angular Electron, 800, scaledwidth="100%"]
...
"scripts": {
  ...
  "pack": "ng build --prod --base-href ./ && electron-builder --dir",
  "dist": "ng build --prod --base-href ./ && electron-builder",
...
To add authentication to your Electron application, please see https://developer.okta.com/blog/2019/03/20/build-desktop-app-with-angular-electron#add-authentication-to-your-angular-electron-desktop-app[Add Authentication to Your Angular Electron Desktop App].

https://github.com/mraible/ng-demo/compare/master\...electron[Click here to see the difference] between this branch and the main branch.
