...
[NOTE]
====
The latest release of Angular CLI (beta 10)
====
...
To navigate to the `SearchComponent`, add a link in `src/app/app.component.html`.
...
In this same file, create a method to gather all the people. Also, define the `Address` and `Person` classes
...
Running `ng test` should result in "Executed 5 of 5 [green]#SUCCESS#". Add a couple more tests for filtering by search term and fetching by id.
...
With mocks in place, you can create the test for `SearchComponent` to use these as providers.
...
You should see "Executed 8 of 8 [green]SUCCESS[/green] (0.238 secs / 0.259 secs)" in the shell window that's running `ng test`. If you don't, try cancelling the command and restarting.
