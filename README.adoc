...
.Protractor with async / await
****
I was https://github.com/angular/protractor/issues/4307#issuecomment-386112285[recently made aware] of a better way to configure Protractor. To integrate these changes into your project, add the following to `e2e/protractor.conf.js`.
...
You'll also need to install @angular/localize to for ng-bootstrap.
...
Change HTML to use Bootstrap classes. For example, change `app.component.html` to have the following HTML:
...
The HTML templates in the `bootstrap4` branch have been changed to use Bootstrap. You can download them using the following command:

----
git clone -b bootstrap4 https://github.com/mraible/ng-demo.git
----
